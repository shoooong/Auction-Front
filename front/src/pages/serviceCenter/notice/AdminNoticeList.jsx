...
const AdminNoticeList = ({ activeTab }) => {
    ...
    useEffect(() => {
        const fetchNoticesAndLuckyDraws = async () => {
            try {
                const response = await jwtAxios.get(`${SERVER_URL}/notice/admin/user/combinedNoticeList`);
                ...
                {filteredNotices.length === 0 && filteredLuckyDraws.length === 0 ? (
                    ...
                    [...filteredNotices, ...filteredLuckyDraws].map((item) => (
                        <li key={item.noticeId || item.luckyAnnouncementId}>
                            <Link to={`/admin/notice/${item.noticeId ? `notice/${item.noticeId}` : `event/${item.luckyAnnouncementId}`}`}>
                                <div>
                                    <span className="type">
